      <div id="toolBarWrap" <?php echo $toolbar=='OFF' ? 'style="display:none"':'' ?>>
        <!-- 사이드바 -->
        <!-- 모바일이 아니거나 레벨이 설정레벨보다 높으면 패스 -->
        <?php if( (!$g['mobile'] && $d['theme']['edit_html'] <= $my['level']) ){?>
        <!-- 레벨이 사진 파일 설정레벨보다 높으면 패스 -->
        <?php if( ($upPhoto <= $my['level']) || ($upFile <= $my['level']) ){?>
        <!-- 다 패스해도 툴바 사용을 껏으면 안보임 -->
        <div id="tx_sidebar" class="tx-sidebar">
          <div class="tx-sidebar-boundary">
            <!-- 사이드바 / 첨부 -->
            <ul class="kimsqDaumEditorToolBar">
              <!-- 레벨이 사진 설정레벨보다 높으면 패스 -->
              <?php if($upPhoto <= $my['level']){?>
              <li>
                <span id="onIcon1" onclick="OpenWindow('<?php echo $g['s']?>/?r=<?php echo $r?>&m=daumEditor&mod=photo');"><img src="<?php echo $g['img_core']?>/_public/ico_photo.gif" alt="" />사진</span>
                <span id="offIcon1" style="display:none;color:#bdbdbd" ><img src="<?php echo $g['img_core']?>/_public/ico_photo.gif" alt="" />사진</span>
              </li>
              <?php }?>
              <!-- 레벨이 파일 설정레벨보다 높으면 패스 -->
              <?php if($upFile <= $my['level']){?>
              <li>
                <span id="onIcon2" onclick="OpenWindow('<?php echo $g['s']?>/?r=<?php echo $r?>&m=daumEditor&mod=file');"><img src="<?php echo $g['img_core']?>/_public/ico_xfile.gif" alt="" />파일</span>
                <span id="offIcon2" style="display:none;color:#bdbdbd" ><img src="<?php echo $g['img_core']?>/_public/ico_xfile.gif" alt="" />파일</span>
              </li>
              <?php }?>
              <!-- 레벨이 사진 설정레벨보다 높으면 패스 -->
              <?php if($upPhoto <= $my['level']){?>
              <li>
                <span id="imgEditor" onclick="runImgEditor();" ><img src="<?php echo $g['img_core']?>/_public/ico_photo.gif" alt="" />포토에디터</span>
                <span id="offIcon3" style="display:none;color:#bdbdbd" ><img src="<?php echo $g['img_core']?>/_public/ico_photo.gif" alt="" />포토에디터</span>
              </li>
              <?php }?>
              <!-- TEXT 모드일 경우 자동 OFF -->
              <li class="tx-list">
              ...
              <li>
                <span id="modeChange" onclick="toggleEditMode();" >[편집모드]</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <!-- 툴바 - 기본 끝 -->
        <!-- 툴바 - 더보기 시작 -->
        <!-- <div id="tx_toolbar_advanced" class="tx-toolbar tx-toolbar-basic"> -->
            ...
            <ul class="tx-bar tx-bar-left">
              ...
              <div id="tx_richtextbox_menu" class="tx-richtextbox-menu tx-menu">
                ...
                <div class="tx-menu-footer">
                  <img class="tx-menu-confirm" src="<?php echo $g['img_module']?>/icon/editor/btn_confirm.gif?rv=1.0.1" alt=""/>
                  <img class="tx-menu-cancel" hspace="3" src="<?php echo $g['img_module']?>/icon/editor/btn_cancel.gif?rv=1.0.1" alt=""/>
                </div>
              </div>
              </li>
              <li class="tx-list">
