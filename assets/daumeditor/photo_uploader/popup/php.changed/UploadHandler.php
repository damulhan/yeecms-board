...
function getFileType($ext)
{
	...
	if (strpos('_doc,xls,ppt,hwp,docx,xlsx,pptx',strtolower($ext))) return 6;
	if (strpos('_zip,tar,gz,tgz,alz,egg',strtolower($ext))) return 7;
	return 1;
}
...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		#error_log(print_r($this->option, true)); 
    }
	...
	public function add_to_database($uploaded_file, $name, $size, $type) 
	{
		global $table; 
		global $DB_CONNECT;
...
#error_log($size);
#error_log($type); 
#error_log($this->get_download_url());
		
		$mingid = getDbCnt($table['bbsupload'],'min(gid)','');
error_log('mingid'. $mingid); 	
		...
		$fserver	= '';
		$url		= $fserver ? $d['upload']['ftp_urlpath'] : '/files/upload/editor/';
		$folder		= $this->get_download_url();
...
error_log($QVAL);
		...
		return db_insert_id($DB_CONNECT);
	}
	
	public function remove_from_database($uploaded_file, $name, $size, $type) 
	{
		global $table; 
		global $DB_CONNECT;
