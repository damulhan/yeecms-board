...
jQuery(function ($) {
                ...
                if (file.url && !file.error) {
                    var ret = othis.get_ratio( file.width, file.height ),
                        ...
                        node
                            ...
                            .end().find(".delete_img").attr(
								{"data-delete":file.name,
								"data-url":file.url, 
								"data-thumbnailUrl":file.thumbnailUrl,								
								"data-deleteurl":file.deleteurl,								
								"data-size":file.size,								
								"data-oriname":file.oriname,								
    ...
    $("#img_upload_submit").bind("click", function(e){
        ...
        $(gnu.dreg_area_list).find(".delete_img").each( function(i, f) {
			...
			attachments['image'].push({
				...
				'data': {
					...
					'filename': $(this).attr("data-delete"),
					'filesize': $(this).attr("data-size"),
					'originalurl': $(this).attr("data-url"),
					'thumburl': $(this).attr("data-thumbnailUrl"),
					//'width': 600,
					'orig_width': $(this).attr("data-width"),
					'orig_height': $(this).attr("data-height"),
        ...
        if( j ) {
			...
			if(opener) {
				var _editor = opener.Editor; 
				if(!_editor) {
					alert('Cannot find opener daumeditor component');
				} 
				
				//var orig_content = _editor.getContent();
				var orig_attachments = _editor.getAttachments();
				
				_editor.modify({
					"attachments": function () {
						var allattachments = orig_attachments;
